...
from .constants import BASE_DIR, DATA_DIR, DB_DIR
        ...
        if os.path.isfile(existing_file_path) and not existing_file.startswith("temp_"):
            with open(existing_file_path, "rb") as ef:
                ...
                if file_hash == existing_hash:
                    ...
                    raise HTTPException(
                        status_code=400, detail="Duplicate file detected."
                    )
...
async def read_file(file_path: str):
    ...
    if (
        ...
        and file_location.is_file()
        and BASE_DIR in file_location.parents
    ):
...
async def embed_file(file_name: str):
    ...
    if (
        ...
        try:
            ...
            db = Chroma(
                persist_directory=f"{DB_DIR}", embedding_function=bge_embeddings
            )
    ...
    return {
        ...
        "embeddings_after": db._collection.count(),
    }
...
async def delete_file(file_path: str):
    ...
    if (
        file_location.exists()
        and file_location.is_file()
        and DATA_DIR in file_location.parents
    ):
        try:
            db = Chroma(
                persist_directory=f"{DB_DIR}", embedding_function=bge_embeddings
